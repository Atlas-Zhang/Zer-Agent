...
Zer-Agent 采用小 package 拆分，让 Provider、Agent 编排、终端 UI 和应用组装可以独立演进。

| Package | 职责 |
| --- | --- |
| `packages/llm-core` | LLM Provider 抽象、DeepSeek/OpenAI-compatible 请求序列化、工具调用标准化、token 用量映射。 |
| `packages/agent-core` | Agent 循环、工具调用生命周期、工具结果处理、最终回答恢复、单轮 token 用量聚合。 |
| `packages/tui` | 终端输入、命令补全、状态栏、颜色、进度渲染和 Markdown 展示。 |
| `packages/app` | CLI 入口、配置加载、会话、内置工具、MCP 工具集成、权限、日志、命令和项目上下文。 |

```mermaid
flowchart TB
  User[用户 / 终端] --> TUI[packages/tui<br/>输入、状态栏、Markdown 渲染]
  TUI --> App[packages/app<br/>CLI 编排、配置、命令]
  App --> Agent[packages/agent-core<br/>Agent 循环和工具事件]
  Agent --> LLM[packages/llm-core<br/>Provider 抽象]
  LLM --> Providers[DeepSeek / OpenAI-compatible APIs]

  Agent --> BuiltInTools[内置工具<br/>文件、搜索、Shell、Git、天气、新闻、TypeScript 符号]
  App --> MCP[MCP stdio Servers<br/>外部工具]
  App --> State[.zer-agent 本地状态<br/>sessions、logs、config、commands、agents]
  BuiltInTools --> LocalSystem[本地仓库<br/>文件系统、Shell、Git]
  BuiltInTools --> ExternalServices[外部 API<br/>Tavily、GNews、Open-Meteo]
```
...
sequenceDiagram
  ...
  User->>TUI: 输入 prompt 或斜杠命令
  TUI->>CLI: 输入文本
  CLI->>CLI: 加载会话、模式、Profile、配置和工具
  CLI->>Agent: 消息 + system prompt + 工具 schema
  Agent->>LLM: Chat completion 请求
  LLM-->>Agent: Assistant 消息或工具调用
  Agent->>CLI: 请求工具授权
  CLI-->>Agent: allow、ask 或 deny
  Agent->>Tool: 执行内置工具或 MCP 工具
  Tool-->>Agent: 工具结果
  Agent->>LLM: 携带工具结果继续请求
  LLM-->>Agent: 最终回答
  CLI->>Store: 保存会话、指标、快照和日志
  CLI->>TUI: 渲染最终回答
```
...
可以在 `.zer-agent/config.json` 中通过 `mcpServers` 配置 MCP stdio server。使用 `/mcp` 查看连接状态。MCP 工具会以 `mcp_<server>_<tool>` 的名称暴露给 Agent。
